...
const StudentDashboard = () => {
  ...
  const handleImageUpload = async (e) => {
    ...
    try {
      // Upload to /api/upload/student/:id
      const res = await axios.post(`http://localhost:5000/api/upload/student/${user._id}`, formData, {
  ...
  const translations = {
    ...
    Marathi: {
      ...
      dashboard: "डॅशबोर्ड", academics: "शैक्षणिक", attendance: "माझी उपस्थिती", results: "परीक्षा निकाल", timetable: "वेळापत्रक", profile: "माझी प्रोफाइल", settings: "सेटिंग्स", logout: "लॉग आउट",
  ...
  useEffect(() => {
    const fetchDashboardData = async () => {
      try {
        ...
        // 2. Fetch Student Profile to get ID, Fees & Language
        ...
        if (myProfile) {
            ...
            // C. Calculate Attendance
            try {
                const attRes = await axios.get(`http://localhost:5000/api/attendance/${myProfile._id}`);
    ...
    <div className={`flex h-screen font-sans overflow-hidden transition-colors duration-300 ${isDark ? 'bg-gray-900 text-white' : 'bg-slate-50 text-slate-800'}`}>
      ...
      <aside className={`w-72 m-4 rounded-3xl flex flex-col shadow-2xl z-20 transition-all duration-300 ${isDark ? 'bg-gray-800' : 'bg-white'}`}>
        ...
        <nav className="flex-1 p-4 space-y-2 overflow-y-auto custom-scrollbar">
          ...
          <SidebarBtn label={t.attendance} icon={<FaChartPie />} active={activeTab === 'attendance'} onClick={() => setActiveTab('attendance')} isDark={isDark} />
          <SidebarBtn label={t.results} icon={<FaClipboardList />} active={activeTab === 'results'} onClick={() => setActiveTab('results')} isDark={isDark} />
          <SidebarBtn label={t.timetable} icon={<FaCalendarAlt />} active={activeTab === 'timetable'} onClick={() => setActiveTab('timetable')} isDark={isDark} />
          <div className={`border-t my-2 ${isDark ? 'border-gray-700' : 'border-gray-100'}`}></div>
          <SidebarBtn label={t.profile} icon={<FaUserGraduate />} active={activeTab === 'profile'} onClick={() => setActiveTab('profile')} isDark={isDark} />
      ...
      <main className="flex-1 p-4 h-screen overflow-y-auto">
        ...
        {activeTab === 'dashboard' && (
           <div className="fade-in space-y-6">
            {/* Banner */}
            <div className="relative overflow-hidden rounded-3xl p-8 bg-gradient-to-r from-blue-600 to-cyan-500 text-white shadow-xl">
              <div className="relative z-10">
                <h1 className="text-3xl font-bold mb-2">{t.hello}, {user.name.split(' ')[0]}! 🚀</h1>
                   ...
                   {loading ? (
                     ...
                     exams.map((exam) => (
                       <ExamItem 
                         ...
                         time={`${exam.startTime} - ${exam.endTime} • Room: ${exam.room || 'TBD'}`} 
                         isDark={isDark} 
                       />
                     ))
                   ) : (
