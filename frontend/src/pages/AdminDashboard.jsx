...
import { 
  ...
  FaPlus, FaBolt, FaSignOutAlt, FaCog, FaUserCircle, FaCamera 
} from 'react-icons/fa';
...
const AdminDashboard = () => {
  ...
  // 1. Handle Profile Picture Upload (Local Update Only)
  const handleImageUpload = async (e) => {
    ...
    try {
      // Since you don't have an Admin model, this backend call might fail if not handled in 'upload.js'.
      // If you want to persist images for admin, you need to create an Admin model backend.
      // For now, we will try to upload, but handle the likely error gracefully.
      
      const res = await axios.post(`http://localhost:5000/api/upload/admin/${user._id}`, formData, {
      ...
      console.error(err);
      alert('Note: Image could not be saved to DB (Admin Model missing), but it might work locally.');
    }
  };
  ...
  return (
    <div className={`flex h-screen transition-colors duration-300 font-sans ${isDark ? 'bg-gray-900 text-white' : 'bg-gray-100 text-gray-800'}`}>
      
      {/* SIDEBAR */}
      <aside className={`w-64 flex flex-col shadow-2xl z-20 transition-colors duration-300 ${isDark ? 'bg-gray-800 border-r border-gray-700' : 'bg-blue-900 text-white'}`}>
        ...
        <nav className="flex-1 p-4 space-y-2 overflow-y-auto custom-scrollbar">
          ...
          <SidebarBtn label={t.admission} icon={<FaPlus />} active={activeTab === 'add-student'} onClick={() => setActiveTab('add-student')} isDark={isDark} />
          <SidebarBtn label={t.fees} icon={<FaMoneyBillWave />} active={activeTab === 'all-students'} onClick={() => setActiveTab('all-students')} isDark={isDark} />
          <SidebarBtn label={t.faculty} icon={<FaChalkboardTeacher />} active={activeTab === 'manage-faculty'} onClick={() => setActiveTab('manage-faculty')} isDark={isDark} />
          <SidebarBtn label={t.courses} icon={<FaBook />} active={activeTab === 'courses'} onClick={() => setActiveTab('courses')} isDark={isDark} />
          
          <div className={`border-t my-2 ${isDark ? 'border-gray-700' : 'border-blue-800'}`}></div>
          
          <SidebarBtn label={t.profile} icon={<FaUserCircle />} active={activeTab === 'profile'} onClick={() => setActiveTab('profile')} isDark={isDark} />
          <SidebarBtn label={t.settings} icon={<FaCog />} active={activeTab === 'settings'} onClick={() => setActiveTab('settings')} isDark={isDark} />
      ...
      <main className="flex-1 p-8 overflow-y-auto">
        {activeTab === 'dashboard' && (
           <div className="fade-in space-y-8 max-w-7xl mx-auto">
            ...
            {/* 1. WELCOME BANNER */}
            <div className={`relative overflow-hidden rounded-3xl p-8 ${isDark ? 'bg-gradient-to-r from-blue-900 to-indigo-900' : 'bg-gradient-to-r from-blue-600 to-indigo-600'} text-white shadow-xl`}>
              <div className="relative z-10">
                <h1 className="text-3xl font-bold mb-2">Welcome Back, {user.name.split(' ')[0]}! 👋</h1>
                <p className="opacity-90">Here's what's happening in your institute today.</p>
              </div>
